...
@app.route('/Marvel')  # Will only accept HTTP GET requests
@authenticate.login_required
def get_marvel_stories():
    if 'story' in request.args:
        ...
        r = requests.get(
            f"http://gateway.marvel.com/v1/public/stories/{request.args['story']}?apikey={marvel_api_public_key}&hash={md5hash}&ts={timestamp}")
